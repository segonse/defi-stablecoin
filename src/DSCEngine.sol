...
contract DSCEngine is ReentrancyGuard {
    ...
    constructor(address[] memory tokenAddresses, address[] memory priceFeedAddress, address dscAddress) {
        ...
        // For example ETH / USD, BTC / USD or MKR / USD
        ...
        moreThanZero(amountDscToBurn)
    {
        ...
        //redeemCollateral has already check _revertIfHealthFactorIsBroken
    }
        ...
        nonReentrant
    {
        ...
        //liquedate Executor will be paid 10%, 0.025ETH * 10% = 0.0025,
        //(0.025 + 0.0025) ETH * 2000 = $55 is _redeemCollateral got
    ...
    /*
     * @dev Low-Level internal funcition, do not call unless the function calling it
    ...
    function getAccountCollateralValueInUsd(address user) public view returns (uint256 totalCollateralValueInUsd) {
        for (uint256 i = 0; i < s_collateralTokens.length; i++) {
            ...
            totalCollateralValueInUsd += getUsdValue(token, amount); //returns定义totalCollateralValueInUsd，函数体内全局
        }
    ...
    function getUsdValue(address token, uint256 amount) public view returns (uint256) {
        ...
        return ((uint256(price) * ADDITONAL_FEED_PRECISION * amount) / 1e18);
    }

    function getTokenAmountFromUsd(address token, uint256 usdAmountInWei) public view returns (uint256) {
        //$75 -> $50 DSC
        //If debtToCover is $50, compute how much ETH is worth $50
        AggregatorV3Interface priceFeed = AggregatorV3Interface(s_priceFeeds[token]);
        (, int256 price,,,) = priceFeed.latestRoundData();
        //$50 * 1e18 // $2000e8 * 1e10 = 0.025ETH
